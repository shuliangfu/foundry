...
[docs/en-US/TEST_REPORT.md](../en-US/TEST_REPORT.md) · **Changelog**: [EN](../en-US/CHANGELOG.md) |
[中文](./CHANGELOG.md)
...
本库经过全面测试，264 个测试用例中 261 个通过，测试覆盖率达到约 80-85%。详细测试报告请查看
...
| 模块               | 测试数 | 说明                     |
| ------------------ | ------ | ------------------------ |
| CLI 命令测试       | 28     | 命令行参数解析、命令识别 |
| Web3 工具函数测试  | 33     | 地址验证、单位转换、哈希 |
| CLI 工具函数测试   | 26     | 配置获取、路径解析       |
| 验证功能测试       | 21     | 合约验证参数、网络配置   |
| 项目初始化测试     | 19     | 目录创建、文件生成       |
| 部署工具函数测试   | 19     | 合约加载、敏感信息过滤   |
| Anvil 时间工具测试 | 17     | 时间同步、时间推进       |
| 缓存功能测试       | 13     | 读写缓存、版本管理       |
| 错误类型测试       | 14     | 错误类继承、上下文信息   |
| Foundry 安装测试   | 13     | 路径查找、自动安装       |
| 配置管理器测试     | 12     | 单例模式、配置加载       |
| Web3 配置加载测试  | 11     | 配置文件、工厂函数       |
| JSR 工具函数测试   | 9      | URL 解析、版本提取       |
| 合约工具测试       | 8      | 合约加载、数据验证       |
| 环境变量工具测试   | 7      | 加载验证、格式处理       |
| 部署功能测试       | 7      | 部署器创建、工具函数     |
| 工具函数测试       | 6      | Logger、合约加载         |
