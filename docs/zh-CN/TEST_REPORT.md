...
- **测试时间**: 2026-07-23
- **测试环境**: Deno 2.9 / Bun 1.3 / Node.js 22（三端交叉验证）
- **数据来源**: 基于实际终端测试输出生成
...
| 运行时         | 通过  | 失败 | 跳过 | 说明                              |
| -------------- | ----- | ---- | ---- | --------------------------------- |
| **Deno 2.9**   | 253   | 0    | 11   | Anvil 测试经 `CI=true` 跳过       |
| **Bun 1.3**    | 225   | 0    | 11   | Anvil 测试经 `CI=true` 跳过       |
| **Node.js 22** | 225   | 0    | 11   | `tsx --test`，Anvil 测试跳过      |

- **通过率**: 100%（三端均 0 失败）✅
- **测试执行时间**: 约 3 秒（Deno）/ 0.5 秒（Bun）/ 0.8 秒（Node）
- **跳过测试**: 11 个（需运行中的 Anvil 节点 `127.0.0.1:8545`；CI 环境经
  `it.skipIf(getEnv("CI") === "true")` 自动跳过）

> **说明**: Deno/Bun/Node 对测试用例的计数方式略有差异（子测试分组），但三端均 0 失败。
> 11 个跳过的测试为 Anvil 集成测试，三端一致。
...
| `cli-utils.test.ts`    | 26     | ✅ 全部通过 | CLI 工具函数测试（扩展）             |
| `cli.test.ts`          | 28     | ✅ 全部通过 | CLI 命令测试（新增）                 |
| `config.test.ts`       | 12     | ✅ 全部通过 | 配置管理器测试                       |
| `contract.test.ts`     | 8      | ✅ 全部通过 | 合约工具测试（新增）                 |
| `deploy-utils.test.ts` | 19     | ✅ 全部通过 | 部署工具函数测试（扩展）             |
| `deploy.test.ts`       | 7      | ✅ 全部通过 | 部署功能测试                         |
| `env.test.ts`          | 7      | ✅ 全部通过 | 环境变量工具测试                     |
| `errors.test.ts`       | 14     | ✅ 全部通过 | 错误类型测试                         |
| `init.test.ts`         | 19     | ✅ 全部通过 | 项目初始化功能测试                   |
| `jsr.test.ts`          | 9      | ✅ 全部通过 | JSR 工具函数测试                     |
| `setup.test.ts`        | 13     | ⏭️ 2 跳过   | Foundry 安装脚本测试（新增）         |
| `time.test.ts`         | 17     | ✅ 全部通过 | Anvil 时间工具测试（新增）           |
| `utils.test.ts`        | 6      | ⏭️ 1 跳过   | 工具函数测试                         |
| `verify.test.ts`       | 21     | ✅ 全部通过 | 验证功能测试（新增）                 |
| `web3-config.test.ts`  | 11     | ✅ 全部通过 | Web3 配置加载测试                    |
| `web3.test.ts`         | 33     | ✅ 全部通过 | Web3 工具函数和实例测试（新增/扩展） |
...
### 2. CLI 工具函数测试 (cli-utils.test.ts) - 26 个测试
...
### 3. CLI 命令测试 (cli.test.ts) - 28 个测试（新增）
...
### 4. 配置管理器测试 (config.test.ts) - 12 个测试
...
### 5. 合约工具测试 (contract.test.ts) - 8 个测试（新增）
...
### 6. 部署工具函数测试 (deploy-utils.test.ts) - 19 个测试
...
### 9. 错误类型测试 (errors.test.ts) - 14 个测试
...
### 10. 项目初始化测试 (init.test.ts) - 19 个测试
...
### 11. JSR 工具函数测试 (jsr.test.ts) - 9 个测试
...
### 12. Foundry 安装脚本测试 (setup.test.ts) - 13 个测试（新增）
...
### 13. Anvil 时间工具测试 (time.test.ts) - 17 个测试（新增）
...
### 15. 验证功能测试 (verify.test.ts) - 21 个测试（新增）
...
### 16. Web3 配置加载测试 (web3-config.test.ts) - 11 个测试
...
### 17. Web3 工具函数和实例测试 (web3.test.ts) - 33 个测试（新增/扩展）
...
| 方法                                                   | 说明                   | 测试覆盖     |
| ------------------------------------------------------ | ---------------------- | ------------ |
| `init(projectRoot?)`                                   | 初始化 Foundry 项目    | ✅ 19 个测试 |
| `deploy(options)`                                      | 部署合约               | ✅ 7 个测试  |
| `verify(options)`                                      | 验证合约               | ✅ 21 个测试 |
...
# Node.js 22
npm install
npm run test:node   # 内部设置 CI=true，使用 tsx --test

# 如有本地 Anvil 节点（运行集成测试）
anvil
...
1. ✅ **全面的测试覆盖**: 覆盖所有公共 API
2. ✅ **新增模块测试**: CLI 命令、验证功能、时间工具、Web3 实例等全面测试
3. ✅ **Anvil 集成测试**: 支持真实 RPC 调用测试（CI 自动跳过）
4. ✅ **三端跨运行时**: Deno 2.9 / Bun 1.3 / Node.js 22 全部通过，基于 @dreamer/runtime-adapter
5. ✅ **错误处理健壮**: 测试验证了各种错误场景的处理机制
6. ✅ **测试清理完善**: afterAll 钩子确保测试后清理临时文件

## 结论

@dreamer/foundry 在**三端运行时**（Deno 2.9 / Bun 1.3 / Node.js 22）上均通过测试，**0 失败**。
11 个 Anvil 集成测试在 CI 自动跳过（需运行中的 Anvil 节点）。

**三端测试结果**:

- **Deno**: 253 通过, 11 跳过, 0 失败 ✅
- **Bun**: 225 通过, 11 跳过, 0 失败 ✅
- **Node.js 22**: 225 通过, 11 跳过, 0 失败 ✅
- **通过率**: 100%（0 失败）
...
- ✅ CLI 命令测试（28 个）- 新增
- ✅ 合约工具测试（8 个）- 新增
- ✅ Foundry 安装脚本测试（13 个）- 新增
- ✅ Anvil 时间工具测试（17 个）- 新增
- ✅ 验证功能测试（21 个）- 新增
- ✅ Web3 工具函数测试（33 个）- 新增/扩展
- ✅ CLI 工具函数测试（26 个）- 扩展
- ✅ 部署工具函数测试（19 个）- 扩展
